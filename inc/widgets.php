...
class kratos_widget_ad extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            ...
            'name'       => __('广告位'),
            'description'=> __('Kratos主题特色组件 - 广告位')
        );
    ...
    function form($instance){
        ...
        @$imgurl = esc_attr($instance['imgurl']); ?>
            <p>
                <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('标题(可留空)：'); ?>
                    <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id('aurl'); ?>"><?php _e('链接：'); ?>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('imgurl'); ?>"><?php _e('图片：'); ?>
...
class kratos_widget_about extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            ...
            'name'       => __('个人简介'),
            'description'=> __('Kratos主题特色组件 - 个人简介')
        );
        parent::__construct(false,false,$widget_ops);
    }
    function widget($args,$instance){
        ...
        <div class="textwidget">
            <div class="widget-admin text-center">
                <p>
                    <a href="<?php echo admin_url('/post-new.php'); ?>"><i class="fa fa-pencil"></i> <?php _e('撰写文章'); ?> </a>
                    <a class="widget-admin-center" href="<?php echo admin_url('/post-new.php?post_type=page'); ?>"><i class="fa fa-plus"></i> <?php _e('新建页面'); ?> </a>
                    <a href="<?php echo admin_url('/edit-comments.php'); ?>"><i class="fa fa-comments"></i> <?php _e('查看评论'); ?></a>
                </p>
                <p>
                    <a href="<?php echo admin_url('/options-general.php'); ?>"><i class="fa fa-cogs"></i> <?php _e('站点设置'); ?> </a>
                    <a class="widget-admin-center" href="<?php echo admin_url('/themes.php?page=kratos'); ?>"><i class="fa fa-cog"></i> <?php _e('主题设置'); ?> </a>
                    <a href="<?php echo wp_logout_url($redirect); ?>"><i class="fa fa-sign-out"></i> <?php _e('退出登录'); ?></a>
                </p>
            </div>
        </div>
        ...
        <div class="textwidget">
            <div class="widget-admin text-center">
                <p>
                    <a href="<?php echo admin_url('/profile.php'); ?>"><i class="fa fa-pencil"></i> <?php _e('个人资料'); ?> </a>
                    <a href="<?php echo admin_url('/'); ?>"><i class="fa fa-dashboard"></i> <?php _e('仪表盘'); ?> </a>
                    <a href="<?php echo wp_logout_url($redirect); ?>"><i class="fa fa-sign-out"></i> <?php _e('退出登录'); ?></a>
                </p>
            </div>
        </div>
    ...
    function form($instance){
        ...
        <p>
            <label for="<?php echo $this->get_field_id('imgurl'); ?>"><?php _e('头像地址：'); ?>
                <input class="widefat" id="<?php echo $this->get_field_id('imgurl'); ?>" name="<?php echo $this->get_field_name('imgurl'); ?>" type="text" value="<?php echo $imgurl; ?>" />
            </label>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('profile'); ?>"><?php _e('简介内容：'); ?>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('bkimgurl'); ?>"><?php _e('卡片背景：'); ?>
...
class kratos_widget_tags extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            'name'       => __('标签聚合'),
            'description'=> __('Kratos主题特色组件 - 标签聚合')
        );
        parent::__construct(false,false,$widget_ops);
    }
    function widget($args, $instance){
    ...
    function form($instance){
        global $wpdb;
        $instance = wp_parse_args((array) $instance,array('title'=>__('标签聚合'),'number'=>'20','orderby'=>'count','order'=>'RAND'));
        $title =  esc_attr($instance['title']);
        $number = intval($instance['number']);
        $orderby =  esc_attr($instance['orderby']);
        $order =  esc_attr($instance['order']); ?>
        <p>
            <label for='<?php echo $this->get_field_id("title"); ?>'><?php _e('标题：'); ?><input type='text' class='widefat' name='<?php echo $this->get_field_name("title"); ?>' id='<?php echo $this->get_field_id("title"); ?>' value="<?php echo $title; ?>"/></label>
        </p>
        <p>
            <label for='<?php echo $this->get_field_id("number"); ?>'><?php _e('数量：'); ?><input type='text' name='<?php echo $this->get_field_name("number"); ?>' id='<?php echo $this->get_field_id("number"); ?>' value="<?php echo $number; ?>"/></label>
        </p>
        <p>
            <label for='<?php echo $this->get_field_id("orderby"); ?>'><?php _e('类型：'); ?>
                <select name="<?php echo $this->get_field_name("orderby"); ?>" id='<?php echo $this->get_field_id("orderby"); ?>'>
                    <option value="count" <?php echo ($orderby=='count')?'selected':''; ?>><?php _e('数量'); ?></option>
                    <option value="name" <?php echo ($orderby=='name')?'selected':''; ?>><?php _e('名字'); ?></option>
                </select>
            </label>
        </p>
        <p>
            <label for='<?php echo $this->get_field_id("order"); ?>'><?php _e('排序：'); ?>
                <select name="<?php echo $this->get_field_name("order"); ?>" id='<?php echo $this->get_field_id("order"); ?>'>
                    <option value="DESC" <?php echo ($order=='DESC')?'selected':''; ?>><?php _e('降序'); ?></option>
                    <option value="ASC" <?php echo ($order=='ASC')?'selected':''; ?>><?php _e('升序'); ?></option>
                    <option value="RAND" <?php echo ($order=='RAND')?'selected':''; ?>><?php _e('随机'); ?></option>
                </select>
            </label>
        </p>
...
class kratos_widget_posts extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            ...
            'name'       => __('文章聚合'),
            'description'=> __('Kratos主题特色组件 - 文章聚合')
        );
        parent::__construct(false,false,$widget_ops);
    }
    function widget($args,$instance){
        ...
        <aside class="widget widget_kratos_poststab">
            <ul id="tabul" class="nav nav-tabs nav-justified visible-lg">
                <li class="active"><span href="#newest" data-toggle="tab"><?php _e('最新文章'); ?></span></li>
                <li><span href="#hot" data-toggle="tab"><?php _e('热点文章'); ?></span></li>
                <li><span href="#rand" data-toggle="tab"><?php _e('随机文章'); ?></span></li>
            </ul>
            <ul id="tabul" class="nav nav-tabs nav-justified visible-md">
                <li class="active"><span href="#newest" data-toggle="tab"><?php _e('最新'); ?></span></li>
                <li><span href="#hot" data-toggle="tab"><?php _e('热点'); ?></span></li>
                <li><span href="#rand" data-toggle="tab"><?php _e('随机'); ?></span></li>
            </ul>
    ...
    function form($instance){
        ...
        $number = intval($instance['number']); ?>
        <p>
            <label for='<?php echo $this->get_field_id("number"); ?>'><?php _e('每项展示数量：'); ?><input type='text' name='<?php echo $this->get_field_name("number"); ?>' id='<?php echo $this->get_field_id("number"); ?>' value="<?php echo $number; ?>"/></label>
        </p>
        <input type="hidden" id="<?php echo $this->get_field_id('submit'); ?>" name="<?php echo $this->get_field_name('submit'); ?>" value="1" /><?php
    }
}
class kratos_widget_comments extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            ...
            'name'       => __('最近评论'),
            'description'=> __('Kratos主题特色组件 - 最近评论')
        );
        parent::__construct(false,false,$widget_ops);
    }
    function widget($args,$instance){
