...
class kratos_local_avatars{
    ...
    <table class="form-table">
        <tr>
            <th><label for="kratos-local-avatar"><?php _e('上传头像','hoshiruri'); ?></label></th>
            ...
            <td><?php
                if(kratos_option('lo_ava')||current_user_can('upload_files')){ ?>
                    ...
                    if(empty($profileuser->kratos_local_avatar)) echo '<span class="description">'.__('尚未设置本地头像，请点击“浏览”按钮上传本地头像。','hoshiruri').'</span>';
                    else echo '
                            <input type="checkbox" name="kratos-local-avatar-erase" value="1" /> '.__('移除本地头像','hoshiruri').'<br />
                            <span class="description">'.__('如需要修改本地头像，请重新上传新头像。如需要移除本地头像，请选中上方的“移除本地头像”复选框并更新个人资料即可。<br/>移除本地头像后，将恢复使用 Gravatar 头像。','hoshiruri').'</span>';      
                }else{
                    if(empty($profileuser->kratos_local_avatar)) echo '<span class="description">'.__('尚未设置本地头像，请在 Gravatar.com 网站设置头像。','hoshiruri').'</span>';
                    else echo '<span class="description">'.__('你没有本地头像上传权限，如需要修改本地头像，请联系站点管理员。','hoshiruri').'</span>';
                } ?>
            </td>
        </tr>
    </table>
    ...
    public function edit_user_profile_update($user_id){
        if(!empty($_FILES['kratos-local-avatar']['name'])){
            ...
            if(strstr($_FILES['kratos-local-avatar']['name'],'.php')) wp_die(__('.php不能出现在文件名中！','hoshiruri'));
            ...
            if(empty($avatar['file'])){
                switch($avatar['error']){
                    case 'Sorry, this file type is not permitted for security reasons.':
                        wp_die(__('请上传jpg,gif,png,bmp,tif格式文件！','hoshiruri'));break;
                    default: wp_die('<strong>'.__('上传头像过程中出现以下错误：','hoshiruri').'</strong> '.esc_attr($avatar['error']));
                }
                return;
            }
