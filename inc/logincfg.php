...
function validdomain($login,$email,$errors){
    ...
    if($isValidEmailDomain===false) $errors->add('domain_error','<strong>'.__('错误','hoshiruri').'</strong>：'.kratos_option('derror'));
}
//Pwd register
...
function kratos_check_extra_register_fields($login,$email,$errors){
    if($_POST['nickname']=='') $errors->add('no_nickname',__("<strong>错误</strong>：昵称一栏不能为空。",'hoshiruri'));
    if($_POST['password']!==$_POST['repeat_password']&&kratos_option('mail_reg')) $errors->add('passwords_not_matched',__("<strong>错误</strong>：两次输入的密码不一致。",'hoshiruri'));
    if(strlen($_POST['password'])<8&&kratos_option('mail_reg')) $errors->add('password_too_short',__("<strong>错误</strong>：密码长度必须大于8位。",'hoshiruri'));
    if($_POST['are_you_human']!=$_POST['num1']+$_POST['num2']) $errors->add('not_human',__("<strong>错误</strong>：验证码错误，请重试。",'hoshiruri'));
}
...
function limit_login_notify_email($user){
    ...
    if(!isset($retries[$ip])){
        ...
        $when = sprintf(__('%d 小时','hoshiruri'),$time);
    }else{
        $count = $retries[$ip];
        $lockouts = floor($count/kratos_option('allowed_retries'));
        $time = round(kratos_option('lockout_duration')/60);
        $when = sprintf(__('%d 分钟','hoshiruri'),$time);
    }
    $blogname = htmlspecialchars_decode(get_option('blogname'),ENT_QUOTES);
    $subject = '['.$blogname.'] '.__('登录失败次数过多','hoshiruri');
    $message = sprintf(__("失败登录次数：%d ，IP：%s",'hoshiruri')
                  ."\r\n\r\n",$count,$ip);
    if ($user != '') {
        $message .= sprintf(__("最后一次尝试登录的用户名：%s",'hoshiruri')
                    ."\r\n\r\n",$user);
    }
    $message .= sprintf(__('此 IP 已被封锁，封锁时长：%s','hoshiruri'),$when);
...
function limit_login_error_msg(){
    $ip = limit_login_get_address();
    $lockouts = get_option('limit_login_lockouts');
    $msg = __('<strong>错误</strong>：登录失败次数过多，','hoshiruri');
    if(!is_array($lockouts)||!isset($lockouts[$ip])||time()>= $lockouts[$ip]){
        $msg .= __('请稍候再试。','hoshiruri');
        return $msg;
    }
    $when = ceil(($lockouts[$ip]-time())/60);
    if($when>60){
        $when = ceil($when/60);
        $msg .= sprintf(__('请在 %d 小时后重试。','hoshiruri'),$when);
    } else {
        $msg .= sprintf(__('请在 %d 分钟后重试。','hoshiruri'),$when);
    }
    return $msg;
}
function limit_login_retries_remaining_msg(){
    ...
    return sprintf(__('<strong>错误</strong>：账号或密码有误，您还有<strong>%d</strong>次尝试机会。','hoshiruri'),$remaining);
}
